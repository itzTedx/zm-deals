...
export const CartItem = ({ item }: Props) => {
  ...
  const generateQuantityOptions = () => {
    ...
    // For combo deals, just show reasonable quantity options
    const options = [];
    for (let i = 1; i <= 10; i++) {
      options.push(i);
    }
  ...
  // Render combo deal item
  if (item.itemType === "combo" && item.comboDeal) {
    const featuredImage = item.comboDeal.images?.find((img) => img.isFeatured) || item.comboDeal.images?.[0];

    return (
      <Card key={item.comboDeal.id}>
        <CardContent className="relative p-3 sm:p-4">
          {isPending && (
            <div className="absolute inset-0 z-10 flex items-center justify-center bg-card/50">
              <div className="size-4 animate-spin rounded-full border-2 border-brand-600 border-t-transparent" />
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:gap-4">
            ...
            <div className="relative size-24 flex-shrink-0 overflow-hidden rounded-lg sm:size-32">
              {featuredImage ? (
                <Image alt={item.comboDeal.title} className="object-cover" fill src={featuredImage.url} />
              ) : (
            ...
            <div className="flex flex-1 flex-col gap-3 sm:flex-row sm:justify-between">
              <div className="space-y-2 sm:space-y-3">
                <h4 className="line-clamp-2 font-medium text-base sm:text-lg">{item.comboDeal.title}</h4>
                {item.comboDeal.description && (
                  <p className="line-clamp-2 text-muted-foreground text-sm">{item.comboDeal.description}</p>
                )}
                <p className="text-muted-foreground text-xs">{item.comboDeal.products?.length || 0} items in combo</p>
              ...
              <div className="flex items-start justify-between gap-3 sm:flex-col sm:items-end">
                ...
                <div className="flex flex-col gap-1 sm:items-end">
                  <div className="flex flex-row items-center gap-2 sm:flex-col sm:items-end">
                    <h5 className="inline-flex items-center gap-0.5 text-xl sm:text-2xl">
                      <IconCurrency className="size-4 sm:size-5" />
                      <b>{item.comboDeal.comboPrice}</b>
                    </h5>

                    <p className="text-gray-400 text-xs">
                      <span className="line-through">{Number(item.comboDeal.originalPrice).toFixed(2)}</span>{" "}
                      <span className="font-medium text-success uppercase">
                        {calculateDiscount(Number(item.comboDeal.originalPrice), Number(item.comboDeal.comboPrice))}%
                        Off
                      </span>
                    </p>
                  </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Select defaultValue={item.quantity.toString()} onValueChange={handleQuantityChange}>
                    <SelectTrigger className="w-14 sm:w-16" id="qty">
                      <SelectValue placeholder="quantity" />
                    </SelectTrigger>
                    <SelectContent className="max-h-72 min-w-6">
